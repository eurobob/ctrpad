...
#define setRECT(v, _x, _y, _w, _h)              (v)->x = (_x), (v)->y = (_y), (v)->w = (_w), (v)->h = (_h)
...
#define setSemiTrans(p, abe)               ((abe) ? (getcode(p) |= 2) : (getcode(p) &= ~2))
#define setSemiTrans_T(p, abe)             ((abe) ? (getcode_T(p) |= 2) : (getcode_T(p) &= ~2))

#define setShadeTex(p, tge)                ((tge) ? (getcode(p) |= 1) : (getcode(p) &= ~1))
#define setShadeTex_T(p, tge)              ((tge) ? (getcode_T(p) |= 1) : (getcode_T(p) &= ~1))

#define getTPage(tp, abr, x, y)            ((((x) & 0x3c0) >> 6) | (((y) & 0x100) >> 4) | (((y) & 0x200) << 2) | (((abr) & 3) << 5) | (((tp) & 3) << 7))
...
#define setLineG3(p)                       setlen(p, 7), setcode(p, 0x58), (p)->pad = 0x55555555, (p)->p1 = 0, (p)->p2 = 0
#define setLineF4(p)                       setlen(p, 6), setcode(p, 0x4c), (p)->pad = 0x55555555
#define setLineG4(p)                       setlen(p, 9), setcode(p, 0x5c), (p)->pad = 0x55555555, (p)->p1 = 0, (p)->p2 = 0, (p)->p3 = 0
#define setFill(p)                         setlen(p, 3), setcode(p, 0x02)
#define setBlit(p)                         setlen(p, 8), setcode(p, 0x80), (p)->pad[0] = 0, (p)->pad[1] = 0, (p)->pad[2] = 0, (p)->pad[3] = 0
...
#define setLineG3_T(p)                     setcode_T(p, 0x58), (p)->pad = 0x55555555, (p)->p1 = 0, (p)->p2 = 0
#define setLineF4_T(p)                     setcode_T(p, 0x4c), (p)->pad = 0x55555555
#define setLineG4_T(p)                     setcode_T(p, 0x5c), (p)->pad = 0x55555555, (p)->p1 = 0, (p)->p2 = 0, (p)->p3 = 0
#define setFill_T(p)                       setcode_T(p, 0x02)
#define setBlit_T(p)                       setcode_T(p, 0x80), (p)->pad[0] = 0, (p)->pad[1] = 0, (p)->pad[2] = 0, (p)->pad[3] = 0

#define setDrawTPage_T(p, dfe, dtd, tpage) (p)->code[0] = (0xe1000000 | (tpage) | (((dtd) & 1) << 9) | (((dfe) & 1) << 10))
#define setDrawTPage(p, dfe, dtd, tpage)   setlen(p, 1), setDrawTPage_T(p, dfe, dtd, tpage)

#define setTexWindow_T(p, r)               (p)->code[0] = (0xe2000000 | ((r)->w & 0x1f) | (((r)->h & 0x1f) << 5) | (((r)->x & 0x1f) << 10) | (((r)->y & 0x1f) << 15))
#define setTexWindow(p, r)                 setlen(p, 1), setTexWindow_T(p, r)

#define setDrawAreaXY_T(p, _x0, _y0, _x1, _y1) \
	(p)->code[0] = (0xe3000000 | ((_x0) & 0x3ff) | (((_y0) & 0x3ff) << 10)), (p)->code[1] = (0xe4000000 | ((_x1) & 0x3ff) | (((_y1) & 0x3ff) << 10))
#define setDrawAreaXY(p, _x0, _y0, _x1, _y1) setlen(p, 2), setDrawAreaXY_T(p, _x0, _y0, _x1, _y1)

#define setDrawArea_T(p, r)                  setDrawAreaXY_T(p, (r)->x, (r)->y, (r)->x + (r)->w - 1, (r)->y + (r)->h - 1)
#define setDrawArea(p, r)                    setlen(p, 2), setDrawArea_T(p, r)

#define setDrawOffset_T(p, _x, _y)           (p)->code[0] = (0xe5000000 | ((_x) & 0x7ff) | (((_y) & 0x7ff) << 11))
#define setDrawOffset(p, _x, _y)             setlen(p, 1), setDrawOffset_T(p, _x, _y)

#define setDrawStp_T(p, pbw, mt)             (p)->code[0] = (0xe6000000 | (pbw) | ((mt) << 1))
#define setDrawStp(p, pbw, mt)               setlen(p, 1), setDrawStp_T(p, pbw, mt)

#define setDrawIRQ_T(p)                      (p)->code[0] = 0x1f000000
#define setDrawIRQ(p)                        setlen(p, 1), setDrawIRQ_T(p)
...
_DEF_PRIM(POLY_F3, uint8_t r0, g0, b0, code; int16_t x0, y0; int16_t x1, y1; int16_t x2, y2;)

_DEF_PRIM(POLY_F4, uint8_t r0, g0, b0, code; int16_t x0, y0; int16_t x1, y1; int16_t x2, y2; int16_t x3, y3;)

_DEF_PRIM(POLY_FT3, uint8_t r0, g0, b0, code; int16_t x0, y0; uint8_t u0, v0; uint16_t clut; int16_t x1, y1; uint8_t u1, v1; uint16_t tpage; int16_t x2, y2;
          uint8_t u2, v2; uint16_t pad;)

_DEF_PRIM(POLY_FT4, uint8_t r0, g0, b0, code; uint16_t x0, y0; uint8_t u0, v0; uint16_t clut; int16_t x1, y1; uint8_t u1, v1; uint16_t tpage; int16_t x2, y2;
          uint8_t u2, v2; uint16_t pad0; int16_t x3, y3; uint8_t u3, v3; uint16_t pad1;)

_DEF_PRIM(POLY_G3, uint8_t r0, g0, b0, code; int16_t x0, y0; uint8_t r1, g1, b1, pad0; int16_t x1, y1; uint8_t r2, g2, b2, pad1; int16_t x2, y2;)

_DEF_PRIM(POLY_G4, uint8_t r0, g0, b0, code; int16_t x0, y0; uint8_t r1, g1, b1, pad0; int16_t x1, y1; uint8_t r2, g2, b2, pad1; int16_t x2, y2;
          uint8_t r3, g3, b3, pad2; int16_t x3, y3;)

_DEF_PRIM(POLY_GT3, uint8_t r0, g0, b0, code; int16_t x0, y0; uint8_t u0, v0; uint16_t clut; uint8_t r1, g1, b1, pad0; int16_t x1, y1; uint8_t u1, v1;
          uint16_t tpage; uint8_t r2, g2, b2, pad1; int16_t x2, y2; uint8_t u2, v2; uint16_t pad2;)

_DEF_PRIM(POLY_GT4, uint8_t r0, g0, b0, code; int16_t x0, y0; uint8_t u0, v0; uint16_t clut; uint8_t r1, g1, b1, pad0; int16_t x1, y1; uint8_t u1, v1;
          uint16_t tpage; uint8_t r2, g2, b2, pad1; int16_t x2, y2; uint8_t u2, v2; uint16_t pad2; uint8_t r3, g3, b3, pad3; int16_t x3, y3; uint8_t u3, v3;
          uint16_t pad4;)

_DEF_PRIM(LINE_F2, uint8_t r0, g0, b0, code; int16_t x0, y0; int16_t x1, y1;)

_DEF_PRIM(LINE_G2, uint8_t r0, g0, b0, code; int16_t x0, y0; uint8_t r1, g1, b1, p1; int16_t x1, y1;)

_DEF_PRIM(LINE_F3, uint8_t r0, g0, b0, code; int16_t x0, y0; int16_t x1, y1; int16_t x2, y2; uint32_t pad;)

_DEF_PRIM(LINE_G3, uint8_t r0, g0, b0, code; int16_t x0, y0; uint8_t r1, g1, b1, p1; int16_t x1, y1; uint8_t r2, g2, b2, p2; int16_t x2, y2; uint32_t pad;)

_DEF_PRIM(LINE_F4, uint8_t r0, g0, b0, code; int16_t x0, y0; int16_t x1, y1; int16_t x2, y2; int16_t x3, y3; uint32_t pad;)

_DEF_PRIM(LINE_G4, uint8_t r0, g0, b0, code; int16_t x0, y0; uint8_t r1, g1, b1, p1; int16_t x1, y1; uint8_t r2, g2, b2, p2; int16_t x2, y2;
          uint8_t r3, g3, b3, p3; int16_t x3, y3; uint32_t pad;)

_DEF_PRIM(TILE, uint8_t r0, g0, b0, code; int16_t x0, y0; int16_t w, h;)

_DEF_PRIM(TILE_1, uint8_t r0, g0, b0, code; int16_t x0, y0;)
_DEF_ALIAS(TILE_8, TILE_1)
_DEF_ALIAS(TILE_16, TILE_1)

_DEF_PRIM(SPRT, uint8_t r0, g0, b0, code; int16_t x0, y0; uint8_t u0, v0; uint16_t clut; uint16_t w, h;)

_DEF_PRIM(SPRT_1, uint8_t r0, g0, b0, code; int16_t x0, y0; uint8_t u0, v0; uint16_t clut;)
_DEF_ALIAS(SPRT_8, SPRT_1)
_DEF_ALIAS(SPRT_16, SPRT_1)

_DEF_PRIM(FILL, uint8_t r0, g0, b0, code; uint16_t x0, y0; uint16_t w, h;)

_DEF_PRIM(DR_MOVE, uint8_t p0, p1, p2, code; uint16_t x0, y0; uint16_t x1, y1; uint16_t w, h;)

_DEF_PRIM(DR_AREA, uint32_t code[2];)
_DEF_PRIM(DR_OFFSET, uint32_t code[1];)
_DEF_PRIM(DR_TWIN, uint32_t code[1];)
_DEF_PRIM(DR_TPAGE, uint32_t code[1];)
_DEF_PRIM(DR_STP, uint32_t code[1];)
_DEF_PRIM(DR_IRQ, uint32_t code[1];)

_DEF_PRIM(DR_ENV, DR_TPAGE_T tpage; DR_TWIN_T twin; DR_AREA_T area; DR_OFFSET_T offset; FILL_T fill;)
